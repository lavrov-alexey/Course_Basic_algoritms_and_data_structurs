...
# v.2 - более лаконичное решение
def even_odd_digits_calc_v2(num, cnt_even=0, cnt_odd=0):
    """Возвращает кортеж с кол-вом четных и нечетных цифр в числе =>
    (cnt_even, cnt_odd)"""

    # базовый случай - все цифры числа извлечены
    if num == 0:
        return cnt_even, cnt_odd  # просто возвращаем накопленные счетчики

    # шаг рекурсии - цифры в числе еще есть
    remain, last_digit = divmod(num, 10)  # отделяем посл. цифру и оставш. число

    # чет/нечет посл. цифры, меняем счетчики, зовем себя же для оставш. числа
    if last_digit % 2 == 0:
        return even_odd_digits_calc_v2(remain, (cnt_even + 1), cnt_odd)
    return even_odd_digits_calc_v2(remain, cnt_even, (cnt_odd + 1))
...
if __name__ == '__main__':
    ...
    even_digits, odd_digits = even_odd_digits_calc_v2(number)
